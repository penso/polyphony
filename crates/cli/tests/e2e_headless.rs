...
fn beads_issues_appear_in_data_issues() {
    ...
    let first = arr
        .iter()
        .find(|i| i["issue_id"].as_str() == Some(&id1))
        .unwrap();
...
fn data_snapshot_returns_complete_structure() {
    ...
    assert!(
        snapshot_is_ready(&snapshot),
        "snapshot not ready after data command"
    );
...
fn issue_cli_round_trips_against_beads() {
    ...
    let created = run_polyphony_json(&repo, &[
        "issue",
        "create",
        "--title",
        "CLI-created issue",
    ...
    let issue = if shown.is_array() {
        shown
            ...
            .unwrap()
            .first()
            .expect("show returned empty array")
            .clone()
    } else {
        shown
    };
    ...
    let updated = run_polyphony_json(&repo, &[
        "issue",
        "update",
        issue_id,
        "--title",
